...
class BuiltInFunction(BaseFunction):
    ...
    def execute_to_number(self, exec_ctx):
        if not (isinstance(exec_ctx.symbol_table.get("value"), String) or isinstance(exec_ctx.symbol_table.get("value"),
                                                                                     Number)):
            return RTResult().failure(RTError(
                self.pos_start, self.pos_end,
...
class Interpreter:
    ...
    def visit_UnaryOpNode(self, node, context):
        ...
        elif node.op_tok.matches(Tokens['KEYWORD'], 'not') or node.op_tok.matches(Tokens['KEYWORD'],
                                                                                  'නොමැත') or node.op_tok.matches(
                Tokens['KEYWORD'], 'නොව') or node.op_tok.matches(Tokens['KEYWORD'], 'නැත') or node.op_tok.matches(
                Tokens['KEYWORD'], 'නොවේ') or node.op_tok.matches(Tokens['KEYWORD'], 'නොව'):
