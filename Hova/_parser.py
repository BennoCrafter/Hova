...
class Parser:
    ...
    def expr(self):
        res = ParseResult()
        if self.current_tok.matches(Tokens['KEYWORD'], 'var') or self.current_tok.matches(Tokens['KEYWORD'],
                                                                                          'විචල්ය') or self.current_tok.matches(
                Tokens['KEYWORD'], 'විචල්‍ය'):
            res.register_advancement()
            self.advance()
    ...
    def comp_expr(self):
        ...
        if self.current_tok.matches(Tokens['KEYWORD'], 'not') or self.current_tok.matches(Tokens['KEYWORD'],
                                                                                          'නොමැත') or self.current_tok.matches(
                Tokens['KEYWORD'], 'නොව') or self.current_tok.matches(Tokens['KEYWORD'],
                                                                      'නැත') or self.current_tok.matches(
                Tokens['KEYWORD'], 'නොවේ') or self.current_tok.matches(Tokens['KEYWORD'], 'නොවන'):
        ...
        Tokens['EQEQ'], Tokens['NOTEQ'], Tokens['LESSTHAN'], Tokens['GREATERTHAN'], Tokens['LTEQ'], Tokens['GTEQ'])))
    ...
    def atom(self):
        ...
        elif tok.matches(Tokens['KEYWORD'], 'function') or tok.matches(Tokens['KEYWORD'], 'ශ්‍රීතය') or tok.matches(
                Tokens['KEYWORD'], 'කාර්යය'):
